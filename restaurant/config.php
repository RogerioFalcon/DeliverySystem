...
// 			die;
			...
			if($json_data['code'] !== 200){
				...
				if( $json_data['msg']['User']['role'] == "hotel" ) { //hotel
					...
					$_SESSION['restaurant_id'] = $json_data['msg']['UserInfo']['user_id'];
...
if( isset($_GET['sendmail']) ) { //register user
    ...
    if( !empty($restarant_name) && !empty($contact_name) && !empty($phone) && !empty($email) && !empty($restarant_address) && !empty($anything_else) ) {
        ...
        $subject = "Phone Number: ".  $data['phone'] . " Restaurant Name: " . $data['restaurant_name'] ;
        $message = "Contact Name: " . $data['contact_name'] . "<br> Address: " . $data['address'] . "<br> Anything Else: "  . $data['description'];
